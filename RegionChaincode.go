...
func (t *RegionChaincode)  RegisterPolicy(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	//err = stub.PutState(RegionName, []byte(strconv.Itoa(InsuredID)), Insured, BusinessArea, []byte(strconv.Itoa(LineOfBusinessID)), Policy, []byte(strconv.Itoa(DealID)) ,DealNum, []byte(strconv.Itoa(BrockerID)), Brocker, CarrierCD, Career)
	...
	// create RegionData object as payload
	//txObj := RegionData{txId, RegionName, InsuredID, Insured, BusinessArea, LineOfBusinessID, Policy, DealID ,DealNum, BrockerID, Brocker, CarrierCD, Career}
	
	//return t.AddTxToHistory(stub, RegionName, txObj)
	//fmt.printf("Transaction successfully added")
	return nil, nil
}
